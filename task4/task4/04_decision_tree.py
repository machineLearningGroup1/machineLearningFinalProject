...
Output:
  - result/decision_tree_log.txt
  - result/dt_best_params.json
  - result/dt_tuning_results.csv
  - result/dt_metrics_summary.json
  - result/dt_classification_report_{train,val,test,full}.txt
  - result/dt_confusion_{train,val,test,full}.png
  - result/dt_feature_importance.png
  - result/dt_tree_plot.png
  - result/dt_decision_boundary_pca.png
  - result/dt_model.pkl
"""
...
sys.stdout = Logger(RESULT_DIR / "decision_tree_log.txt")
...
def save_report(y_true: pd.Series, y_pred: np.ndarray, split_name: str) -> None:
    ...
    report_path = RESULT_DIR / f"dt_classification_report_{split_name}.txt"
...
def plot_confusion(y_true: pd.Series, y_pred: np.ndarray, split_name: str) -> None:
    ...
    disp.ax_.set_title(f"Decision Tree - Confusion Matrix ({split_name})")
    plt.tight_layout()
    plt.savefig(RESULT_DIR / f"dt_confusion_{split_name}.png", bbox_inches="tight")
    plt.close()
...
def manual_grid_search(
    ...
    print("=" * 70)
    print("Task 4 - Decision Tree Classification")
    print("=" * 70)
    ...
    result_df = pd.DataFrame(records).sort_values(
        by=["val_macro_f1", "val_accuracy", "tree_depth"],
        ascending=[False, False, True],
    )
    result_df.to_csv(RESULT_DIR / "dt_tuning_results.csv", index=False)
    ...
    print(f"  Best validation macro-F1: {best_record['val_macro_f1']:.4f}")
...
def plot_feature_importance(
    ...
    ax.set_title(f"Decision Tree - Top {top_n} Feature Importances")
    ax.set_xlabel("Importance")
    plt.tight_layout()
    plt.savefig(RESULT_DIR / "dt_feature_importance.png", bbox_inches="tight")
    plt.close()
...
def plot_tree_figure(model: DecisionTreeClassifier, feature_names: list[str]) -> None:
    ...
    ax.set_title("Decision Tree (top 4 levels only)")
    plt.tight_layout()
    plt.savefig(RESULT_DIR / "dt_tree_plot.png", bbox_inches="tight")
    plt.close()
...
def plot_decision_boundary_pca(
    ...
    print("\n[STEP 6] PCA decision boundary visualization")
    ...
    ax.set_title("Decision Tree Decision Boundary on PCA-2D Space")
    ax.set_xlabel("PCA component 1")
    ax.set_ylabel("PCA component 2")
    ax.legend(framealpha=0.8)
    plt.tight_layout()
    plt.savefig(RESULT_DIR / "dt_decision_boundary_pca.png", bbox_inches="tight")
    plt.close()
...
with open(RESULT_DIR / "dt_best_params.json", "w", encoding="utf-8") as f:
...
joblib.dump(final_model, RESULT_DIR / "dt_model.pkl")
...
with open(RESULT_DIR / "dt_metrics_summary.json", "w", encoding="utf-8") as f:
...
# ============================================================
print("\n[STEP 4] Save feature importance plot")
plot_feature_importance(final_model, X_dev.columns.tolist(), top_n=20)

print("\n[STEP 5] Save tree structure plot")
...
# ============================================================
print("\n" + "=" * 70)
print("Decision Tree task completed")
print("=" * 70)
print("Saved files:")
print("  - dt_best_params.json")
print("  - dt_tuning_results.csv")
print("  - dt_metrics_summary.json")
print("  - dt_classification_report_train.txt")
print("  - dt_classification_report_val.txt")
print("  - dt_classification_report_test.txt")
print("  - dt_classification_report_full.txt")
print("  - dt_confusion_train.png")
print("  - dt_confusion_val.png")
print("  - dt_confusion_test.png")
print("  - dt_confusion_full.png")
print("  - dt_feature_importance.png")
print("  - dt_tree_plot.png")
print("  - dt_decision_boundary_pca.png")
print("  - dt_model.pkl")
print("  - decision_tree_log.txt")
